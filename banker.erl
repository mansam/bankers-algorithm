...
loop(Capital, Clients, Defered) ->
	receive
		...
		{Pid, detach_} ->
			...
			case Result of
				{ok, NewClients} 	-> Pid ! {ok},
										{ResolvedDeferals, NewDefered} = check_defered(Capital, NewClients, Defered),
		...
		{Pid, request_, NUnits} ->
			...
			case Result of
				...
				{defer, Message}	-> io:format("~p~n", [Message]), loop(Capital, Clients, [{Pid, NUnits} | Defered]);
				Any					-> Pid ! Any, loop(Capital, Clients, Defered)
			end;
		{Pid, release_, NUnits} ->
...
h_check_defered(Capital, Clients, [ {Pid, NUnits} | T], NewDefered) ->
	...
	Result = check_safety(Capital, Remaining ++ [{Pid, Limit, Loan + NUnits}], NUnits),
	case Result of
		safe 	-> Pid ! {ok},
					io:format("Banker: Granting delayed request for ~p to ~p.~n", [NUnits, Pid]),
					h_check_defered(Capital, Remaining ++ [{Pid, Limit, Loan + NUnits}], T, NewDefered);
		unsafe 	-> h_check_defered(Capital, Clients, T, [ {Pid, NUnits} | NewDefered ])
	end.
...
h_request(_Capital, _NewClients, [], _Pid, _NUnits) -> {error, "Banker: Client not attached."};
...
h_release(_Capital, _NewClients, [], _Pid, _NUnits) -> {error, "Banker: Client not attached."};
...
% Attach a new client to the bank.
...
h_attach(_NewClients, [{Pid, _L, _Loan} | _T], Pid, _Limit) -> {error, "Banker: Already attached."};
...
h_detach(_NewClients, [], _Pid) -> {error, "Banker: Client not attached."};
